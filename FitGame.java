...
public class FitGame{
  ...
  private Map<Character,GeneticBayesAgent> players;
  ...
  /**
	Creates a game between agents from the genomes in the list
	@param genomes an array list of genomes to be used
	**/
	public FitGame(ArrayList<Genome> gen)
	{
		init();
		this.genomes = gen;
	}
   ...
   * */
  private void init(){
    players = new HashMap<Character,GeneticBayesAgent>();
   ...
   * */
  public void addPlayer(GeneticBayesAgent a){
  ...
  public Map<Integer,Integer> play(){
	...
	int sr, rr;
	if(fails>2)  //Set spy reward and resistance reward
	{
		sr = 1;
		rr = 0;
	}
	...
	for(char c : playerString.toCharArray())
	{
		if(spyString.indexOf(c) != -1)
			result.put(players.get(c).getID(),sr);
		else
			result.put(players.get(c).getID(),rr);
	}
	return result;
  }
   ...
   **/
	...
	{
		char c = 'A';
		FitGame f = new FitGame(gens);
		for(int i=0; i<gens.size(); i++)
		{
			f.stopwatchOn();
			f.addPlayer(new GeneticBayesAgent(gens.get(i)));
			f.stopwatchOff(1000,'A');
		}
		f.setup();
		return f.play();
  }
